...
const OrderPrice = () => {
  ...
  for (var i = 0; i < order.length; i++) {
    allPrice += order[i].fields.preco?.stringValue * order[i].quantity
  ...
  return (
    <div className="my-4 flex flex-col space-y-3">
      ...
      <div className="flex items-center">
        <span className="poppins flex-grow text-xl text-gray-700">Total</span>
        <span className="poppins text-xl font-semibold text-black">
          R$ {total.toFixed(2)}
        </span>
      </div>
    </div>
