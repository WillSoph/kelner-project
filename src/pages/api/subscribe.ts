...
export default async (req: NextApiRequest, res: NextApiResponse) => {
  try {
    if (req.method === 'POST') {
      ...
      const stripeCustomerId = await criarStripeCustomerOuObterExiste(
        currentUser,
        uidUser
      )
      ...
      const stripeCheckoutSession = await stripe.checkout.sessions.create({
        ...
        line_items: [{ price: 'price_1O0UrrIYJ05oSoaZonPhWe4G', quantity: 1 }],
        mode: 'subscription',
...
async function criarStripeCustomerOuObterExiste(email, uidUser) {
  ...
  if (!querySnapshot.empty) {
    ...
    if (userData.stripe_customer_id) {
      console.log(
        'Usuário já possui stripe_customer_id:',
