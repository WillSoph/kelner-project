...
const ImprimirCardapio = React.forwardRef((props, ref) => {
  ...
  return (
    <>
      ...
      A título de transparência, a Menu Simples poderá modificar as regras citadas abaixo visando melhorias em seus sistemas. Portanto, recomendamos que a leitura de nossa política de privacidade seja realizada de forma periódica.
      <br />
      <h2>Tipo de informação armazenada</h2>
      Quando você cria uma conta no site Menu Simples, capturamos informações como nome do responsável, e-mail, telefone, endereço, CNPJ. 

      O sistema também armazena dados cadastrais, preferências e histórico de compra de clientes que se cadastraram no site. Esses dados apenas podem ser consultados pelo próprio cliente, mediante identificação por senha. Dessa forma, os dados pessoais de clientes estarão protegidos contra o uso indevido por terceiros e desde já fica consentido que os mesmos poderão ser tratados exclusivamente pelo Menu Simples, nos termos da Lei Geral de Proteção de Dados – LGPD.
      <br />
      ...
      <br />
      <h2>Segurança</h2>
      O Menu Simples utiliza a Google Cloud, um dos maiores datacenters do mundo. Com ISO 27001, provê várias camadas de segurança física e operacional. Possui redundância de serviços e estrutura incluindo DNS, servidores e storage. CDN distribuído por várias regiões do globo para entrega de conteúdo com baixa latência.
      <br />
      <h2>Utilização dos dados armazenados</h2>
      Utilizamos seus dados pessoais para enviar avisos importantes, como comunicados da ferramenta, changelog, condições e políticas, informações sobre o pacote do plano adquirido, entre outras. Como estas informações são importantes para a sua interação com o Menu Simples. Por este termo o cliente manifesta seu total e expresso consentimento para que o Menu Simples utilize seus dados pessoais exclusivamente para a finalidade abaixo, sendo vedado o compartilhamento com terceiros.
      ...
      Nosso site poderá conter links para outros sites não pertencentes à Menu Simples. Nesses casos, a Menu Simples não é responsável pelas ações desses sites, para os quais não será aplicável a presente Política de Privacidade.

      A Menu Simples é titular de direitos sobre todos os bancos de dados e dispositivos de armazenamento de dados, bem como sobre os softwares de que utiliza. No entanto, a Menu Simples não será titular das informações do usuário armazenadas no site e/ou recebidas em decorrência das vendas realizadas através do site, as quais sempre pertencerão aos usuários.
      <br />
...
export default ImprimirCardapio;
